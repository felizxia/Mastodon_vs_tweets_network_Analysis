...
def follow_relationship_write(file):
    ...
    dict = {}
    for file in os.listdir('/Users/rita/Google Drive/608/gragh/instance_community/accounts'): # change path
...
def degree_distribution_cdf(file):
    ...
    if file == "instance_pairs_in":
        dict_fr = defaultdict(int)
        for i in list(pair.in_degree().values()):
            dict_fr[i] += 1
        items = sorted(dict_fr.items(), reverse=True)
        ...
        plt.show()
    if file == "instance_pairs_out":
        dict_fr_out = defaultdict(int)
        fig = plt.figure()
        ax = fig.add_subplot(111)
        for i in list(pair.out_degree().values()):
            dict_fr_out[i] += 1
        items = sorted(dict_fr_out.items(),reverse=True)
        x, y = np.array(items).T
        y = [float(i) / sum(y) for i in y]
        y = cumsum(y)
        plt.plot(x, y, 'bo')
